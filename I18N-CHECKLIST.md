...
- ~~**Home destination-picker country names**~~ ✅ Fixed in PR [#38](https://github.com/Anil682003/sunsky-website/pull/38). Reported live by the client: picking a city from the Home page's main destination picker (e.g. Kusadasi) showed "Verblijven in Kusadasi, Turkey" — `selectionLabel()` in `Hero.jsx` built the "City, Country" text (which also becomes the results page's `destinationLabel` URL param, trusted verbatim) from the raw backend country name, never routing it through `countryName()`/`Intl.DisplayNames` like the country-only search path already did. Found and fixed the identical bug in `HolidayType.jsx`'s city-card country subtitle and country-card name/label/href.
